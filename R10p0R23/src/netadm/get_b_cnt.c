 ...
 *
 * Module Function:
 * 	A RPC call to return the number of B channels on an Annex.
 *
...
get_b_count(Pinet_addr, type, Pdata)
...

{
    ...
    return rpc(Pinet_addr, RPROC_GET_BS, OUTGOING_COUNT, outgoing,
	       Pdata, type);

}   /* get_b_count() */

get_all_b_counts(Pinet_addr, type, Pdata)
    struct sockaddr_in *Pinet_addr;
    u_short         type;
    char            *Pdata;

{
    ...
    /* Call rpc() to communicate the request to the annex via erpc or srpc. */
    return rpc(Pinet_addr, RPROC_GET_ALL_BS, OUTGOING_COUNT, outgoing,
	       Pdata, type);
}
